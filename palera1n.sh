...
get_device_mode() {
    ...
    if [ "$device_count" = "0" ]; then
        ...
        echo "[-] Please attach only one idevice" > /dev/tty
        kill -30 0
        exit 1;
    fi
    if [ "$os" = "Linux" ]; then
...
_wait() {
    if [ "$(get_device_mode)" != "$1" ]; then
        echo "[*] Waiting for idevice in $1 mode"
    fi
...
_dfuhelper() {
    ...
    if [ "$(get_device_mode)" = "dfu" ]; then
        ...
        echo "[-] idevice did not enter DFU mode, rerun the script and try again"
        return -1
    fi
}
...
if [ ! -e "$dir"/gaster ]; then
    curl -k -sLO https://nightly.link/pwnd2e/gaster-3.0/workflows/makefile/main/gaster-"$os".zip
...
echo "Written by Nebula and Mineek | Some code and ramdisk from Nathan | Loader app by Amy | this fork by pwnd2e"
echo ""
...
echo $(echo "[*] Detected $(get_device_mode) mode idevice" | sed 's/dfu/DFU/')

if grep -E 'pongo|checkra1n_stage2|diag' <<< "$(get_device_mode)"; then
    echo "[-] Detected idevice in unsupported mode '$(get_device_mode)'"
    exit 1;
fi

if [ "$(get_device_mode)" != "normal" ] && [ -z "$version" ] && [ "$dfuhelper" != "1" ]; then
    echo "[-] You must pass the version your idevice is on when not starting from normal mode"
    exit
fi
...
if [ "$(get_device_mode)" = "ramdisk" ]; then
    ...
    _kill_if_running iproxy
    echo "[*] Rebooting idevice in SSH Ramdisk"
    if [ "$os" = 'Linux' ]; then
...
if [ "$(get_device_mode)" = "normal" ]; then
    ...
    if [ "$arch" = "arm64e" ]; then
        echo "[-] palera1n doesn't, and never will, work on non-checkm8 idevices"
        exit
    fi
...
if [ ! -f blobs/"$deviceid"-"$version".shsh2 ]; then
    ...
    if [ "$semi_tethered" = "1" ]; then
        if [ -z "$skip_fakefs" ]; then
            echo "[*] Creating fakefs, this may take a while (up to 10 minutes please wait)"
